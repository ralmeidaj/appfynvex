...
import {iniciarAssinaturaContratoMae, confirmarAssinaturaContratoMae} from '../../api/antecipacoes';

type Nav = NativeStackNavigationProp<InicioStackParamList>;

// Mesmas 3 fases de CadastroFacialEnrollScreen/RecuperarFacialScreen —
// RF-MAE-02: mesmo mecanismo de segundo fator de RF-BIO-03.
type Phase = 'iniciando' | 'capturando' | 'confirmando' | 'erro';

export function AdvanceMaeFacialConfirmScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await iniciarAssinaturaContratoMae();
        if (!active) {
          return;
        }
  ...
  const handleSimulateCapture = useCallback(() => {
    ...
    setTimeout(async () => {
      try {
        await confirmarAssinaturaContratoMae(sessionId);
    ...
    <SafeAreaView style={styles.safe}>
      <View style={styles.content}>
        ...
        <Text style={styles.subtitle}>
          Por segurança, confirme a assinatura do Contrato-Mãe com a leitura facial (2º fator).
        </Text>
